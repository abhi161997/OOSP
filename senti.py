...
#print("negetive",vs["neg"])
#print("neutral",vs["neu"])
#print("positive",vs["pos"])
...
cur = db.cursor()
sql = "select * from p_review order by review_id desc limit 1"
cur.execute(sql)
data = cur.fetchall()
i = 0
for row in data:
    review = row[1]
    r_id = row[0]
    rscore = analyzer.polarity_scores(review)
    rneg = rscore["neg"]
    rneu = rscore["neu"]
    rpos = rscore["pos"]
    rcom = rscore["compound"]*10
    sql1 = "update p_review set rating = "+str(rcom)+" where review_id = "+str(r_id)
    cur.execute(sql1)
    db.commit()
cur.close()
